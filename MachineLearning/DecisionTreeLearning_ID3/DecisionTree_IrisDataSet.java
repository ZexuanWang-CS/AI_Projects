...
public class DecisionTree_IrisDataSet {
	...
	public DecisionTree_IrisDataSet() {
		this.Root = null;
	}

	public static void main(String[] args) throws IOException {
		DecisionTree_IrisDataSet DeciTree = new DecisionTree_IrisDataSet();
		...
		try (PrintWriter pW = new PrintWriter(File)) {
			...
			ArrayList<String[]> IrisTrain = new ArrayList<String[]>();
			ArrayList<String[]> IrisTrainSub = new ArrayList<String[]>();
			for (int i = 0; i < DeciTree.Iris.size(); i++) {
				if (i < DeciTree.Iris.size()/10) {
					IrisTest.add(DeciTree.Iris.get(i));
				}
				else {
					IrisTrain.add(DeciTree.Iris.get(i));
				}
			}
			for (int m = 1; m <=90; m++) {
				IrisTrainSub.clear();
				for (int n = 0; n < (m*IrisTrain.size()/90); n++) {
					IrisTrainSub.add(IrisTrain.get(n));
				}
				pW.println(DeciTree.Error(IrisTrainSub, IrisTest));
			}
		}
		catch (FileNotFoundException e) {
			e.printStackTrace();
